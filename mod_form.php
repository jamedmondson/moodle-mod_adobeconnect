...
//-------------------------------------------------------------------------------
    ...
    function validation($data, $files) {
        ...
        if (empty($data['update'])) {
            ...
            // Need to be aware that groupings might not be turned on
            if (!isset($data['groupingid']) || !$data['groupingid']) {
                $groupingid = 0;
            } else {
                $groupingid = $data['groupingid'];
            }

            // Check that the course (or selected grouping) has groups if group mode is on
            if (0 != $data['groupmode']) { // Only for group mode
                // get all groups for the course
                $crsgroups = groups_get_all_groups($COURSE->id, 0, $groupingid);
                if (empty($crsgroups)) {
            ...
            if (0 != $data['groupmode']) { // Only for group mode
                // get all groups for the course
                $crsgroupsmaxnamelength = groups_get_all_groups($COURSE->id, 0, $groupingid);
